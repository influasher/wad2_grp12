// THIS FILE IS NOT IN USE, EVERYTHING MOVED OVER TO LABGAMESCENE.JS
...
class LabGameScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('testTubePrecipitateOrange', 'assets/images/orangeTestTube.png');
    ...
    //   Optionally, you can change the button's appearance when hovering or clicking
        ...
        this.testTubes = [
            this.add.image(this.scale.width * 0.35, this.scale.height * 0.7, 'testTubeEmpty').setScale(this.scale.width * 0.004),
            this.add.image(this.scale.width * 0.45, this.scale.height * 0.7, 'testTubeEmpty').setScale(this.scale.width * 0.004),
            this.add.image(this.scale.width * 0.55, this.scale.height * 0.7, 'testTubeEmpty').setScale(this.scale.width * 0.004),
        ];

        // Define precipitate colors (red, blue, green, white, orange)
        const precipitateColors = [
            'testTubePrecipitateRed',
            'testTubePrecipitateBlue',
            'testTubePrecipitateGreen',
            'testTubePrecipitateWhite',
            'testTubePrecipitateOrange'
        ];

        // Shuffle and assign a color to each test tube randomly
        const shuffledColors = Phaser.Utils.Array.Shuffle(precipitateColors);
        this.testTubeColors = shuffledColors.slice(0, 3); // Assign 3 colors randomly
        ...
        this.testTubes.forEach(testTube => {
            testTube.setInteractive({ dropZone: true });
        });
        ...
        this.input.on('drop', (pointer, gameObject, dropZone) => {
            ...
            if (testTubeIndex !== -1 && !this.precipitateFormed[testTubeIndex]) {
                const bottleName = this.getBottleName(gameObject);
                if (bottleName === 'Sodium Hydroxide') {
                    this.addPrecipitateToTestTube(testTubeIndex);
                } else {
                    this.instructionText.setText('No visible reaction.');
                }
            }
    ...
    addPrecipitateToTestTube(testTubeIndex) {
        if (!this.precipitateFormed[testTubeIndex]) {
            const selectedColor = this.testTubeColors[testTubeIndex];
            const stream = this.add.image(this.testTubes[testTubeIndex].x, this.testTubes[testTubeIndex].y - 50, 'pouringStream').setDisplaySize(30, 100);
            ...
            this.tweens.add({
                ...
                onComplete: () => {
                    ...
                    this.instructionText.setText(`${selectedColor.replace('testTubePrecipitate', '')} precipitate formed.`);
